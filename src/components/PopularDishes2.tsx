          ...
          {popularDishesData.map((dish, i) => (
            <div 
              ...
              onClick={() => {
                ...
                const route = categoryRouteMap[key] || key.replace(/\s+/g, '-');
                navigate('/menu/' + route);
              }}
            ...
            >
              ...
              />
              <div className="absolute inset-0 bg-linear-to-t from-black/90 via-black/20 to-black/20 group-hover:from-black/80 transition-colors duration-500 z-0"></div>
              ...
              <div className="absolute bottom-4 md:bottom-6 left-4 md:left-6 right-4 md:right-6 border border-white/20 bg-black/40 backdrop-blur-xl rounded-3xl md:rounded-4xl p-4 md:p-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 z-20 flex flex-col justify-end translate-y-4 group-hover:translate-y-0">
